...
typedef Eigen::Vector3d ObsSize;
typedef pair<ObsPos, ObsPos> Obstacle;
std::vector<Obstacle> obstacle_list;

void
fixedMapGenerate(rclcpp::Logger logger)
{
  ...
  RCLCPP_WARN(logger, "Finished generate random map ");
...
void
rcvOdometryCallbck(const nav_msgs::msg::Odometry::SharedPtr odom)
{
...
void
pubSensedPoints(rclcpp::Node::SharedPtr node)
{
  if (!map_ok || !_has_odom)
    return;
  ...
  auto time_aft_sensing = node->now();
  ...
  if (frequence_division_global == 0)
  {
    ...
    RCLCPP_INFO(node->get_logger(), "[SERVER]Publish one global map");
  }
}
