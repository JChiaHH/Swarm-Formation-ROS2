...
SwarmGraph::SwarmGraph()
    : logger_(rclcpp::get_logger("swarm_graph"))
{
    have_desired = false;
}
...
bool SwarmGraph::updateGraph( const std::vector<Eigen::Vector3d> &swarm){
    ...
    if(nodes.size() != nodes_des.size()){
        ...
        RCLCPP_WARN(logger_, "Size of swarm formation vector is incorrect. ");
        return false;
    }
    ...
    if( have_desired ){
        ...
        RCLCPP_WARN(logger_, "Please enter the desired formation!!");
    }
...
bool SwarmGraph::calcFNorm2( double &cost ){
    ...
    if( have_desired ){
        ...
        RCLCPP_WARN(logger_, "Invalid desired formation.");
        return false;
    }
}
...
Eigen::Vector3d SwarmGraph::getGrad(int id){
    if (have_desired && id<(int)nodes_des.size()){
        return agent_grad[id];
    } else {
        Eigen::Vector3d grad = Eigen::Vector3d::Zero();
        RCLCPP_WARN(logger_, "id is error !!! or desired graph has not been setup !!!");
        return grad;
    }
...
bool SwarmGraph::getGrad(std::vector<Eigen::Vector3d> &swarm_grad){
    if (have_desired){
        ...
        RCLCPP_WARN(logger_, "desired graph has not been setup !!!");
        return false;
    }
}
