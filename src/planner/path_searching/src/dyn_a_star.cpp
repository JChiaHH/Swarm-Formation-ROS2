...
bool AStar::ConvertToIndexAndAdjustStartEndPoints(Vector3d start_pt, Vector3d end_pt, Vector3i &start_idx, Vector3i &end_idx)
{
    ...
    {
        //RCLCPP_WARN(..., "Start point is inside an obstacle.");
        do
        {
    ...
    {
        //RCLCPP_WARN(..., "End point is inside an obstacle.");
        do
        {
...
bool AStar::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt, bool use_esdf_check)
{
    ...
    {
        RCLCPP_ERROR(rclcpp::get_logger("path_searching"), "Unable to handle the initial or end point, force return!");
        return false;
    }
    ...
    while (!openSet_.empty())
    {
        ...
        if (current->index(0) == endPtr->index(0) && current->index(1) == endPtr->index(1) && current->index(2) == endPtr->index(2))
        {
            ...
            printf("\033[34mA star iter:%d, time:%.3f\033[0m\n",num_iter, (time_2 - time_1).seconds()*1000);
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("path_searching"), "Failed in A star path searching !!! 0.2 seconds time limit exceeded.");
            return false;
        }
    }
    ...
    if ((time_2 - time_1).seconds() > 0.1)
        RCLCPP_WARN(rclcpp::get_logger("path_searching"), "Time consume in A star path finding is %.3fs, iter=%d", (time_2 - time_1).seconds(), num_iter);
...
vector<Vector3d> AStar::astarSearchAndGetSimplePath(const double step_size, Vector3d start_pt, Vector3d end_pt){
    ...
    if ((path[0]-start_pt).norm() > 0.5){
        RCLCPP_WARN(rclcpp::get_logger("path_searching"), "I don't know why, but only try A* again");
    ...
    int size = path.size();
    if (size <= 2){
        RCLCPP_WARN(rclcpp::get_logger("path_searching"), "the path only have two points");
        return path;
    }
