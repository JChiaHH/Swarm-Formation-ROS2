...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(rclcpp::Node::SharedPtr node, PlanningVisualization::Ptr vis)
  {
    ...
    if (!node_->has_parameter("manager/max_vel")) {
      node_->declare_parameter<double>("manager/max_vel", -1.0);
    }
    node_->get_parameter("manager/max_vel", pp_.max_vel_);
    if (!node_->has_parameter("manager/max_acc")) {
      node_->declare_parameter<double>("manager/max_acc", -1.0);
    }
    node_->get_parameter("manager/max_acc", pp_.max_acc_);
    if (!node_->has_parameter("manager/feasibility_tolerance")) {
      node_->declare_parameter<double>("manager/feasibility_tolerance", 0.0);
    }
    node_->get_parameter("manager/feasibility_tolerance", pp_.feasibility_tolerance_);
    if (!node_->has_parameter("manager/control_points_distance")) {
      node_->declare_parameter<double>("manager/control_points_distance", -1.0);
    }
    node_->get_parameter("manager/control_points_distance", pp_.ctrl_pt_dist);
    if (!node_->has_parameter("manager/polyTraj_piece_length")) {
      node_->declare_parameter<double>("manager/polyTraj_piece_length", -1.0);
    }
    node_->get_parameter("manager/polyTraj_piece_length", pp_.polyTraj_piece_length);
    if (!node_->has_parameter("manager/planning_horizon")) {
      node_->declare_parameter<double>("manager/planning_horizon", 5.0);
    }
    node_->get_parameter("manager/planning_horizon", pp_.planning_horizen_);
    if (!node_->has_parameter("manager/use_distinctive_trajs")) {
      node_->declare_parameter<bool>("manager/use_distinctive_trajs", false);
    }
    node_->get_parameter("manager/use_distinctive_trajs", pp_.use_distinctive_trajs);
    if (!node_->has_parameter("manager/drone_id")) {
      node_->declare_parameter<int>("manager/drone_id", -1);
    }
      ...
      const bool use_formation, const bool have_local_traj)
  {
    ...
    {
      traj_.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr()->getTraj(), node_->now().seconds()); // todo time
    }
  ...
  bool EGOPlannerManager::EmergencyStop(Eigen::Vector3d stop_pos)
  {
    ...
    traj_.setLocalTraj(stopMJO.getTraj(), node_->now().seconds());
