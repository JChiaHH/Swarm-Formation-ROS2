...
namespace ego_planner
{
      ...
      Eigen::MatrixXd &optimal_points, const bool use_formation)
  {
    ...
    std::vector<double> q_vec(variable_num_);
    double *q = q_vec.data();
    ...
    printf("\033[32miter=%d, use_formation=%d, formation_size=%d, time(ms)=%5.3f, result=%s, occ=%d, drone_id=%d\n\033[0m",
           iter_num_, use_formation, formation_size_, time_ms, lbfgs::lbfgs_strerror(result), (int)occ, drone_id_);
  ...
  double PolyTrajOptimizer::costFunctionCallback(void *func_data, const double *x, double *grad, const int n)
  {
    ...
    opt->iter_num_ += 1;
    // Debug: print formation cost every 20 iterations
    if (opt->iter_num_ % 20 == 1 && opt->use_formation_) {
      printf("[FORMATION_DBG] drone=%d iter=%d obs=%.1f swarm=%.1f formation=%.4f feas=%.1f var=%.1f\n",
             opt->drone_id_, opt->iter_num_,
             obs_swarm_feas_qvar_costs(0), obs_swarm_feas_qvar_costs(1),
             obs_swarm_feas_qvar_costs(2),
             obs_swarm_feas_qvar_costs(3) + obs_swarm_feas_qvar_costs(4),
             obs_swarm_feas_qvar_costs(5));
    }
                                              ...
                                              double &costp)
  {
    ...
    int traj_size = (int)swarm_trajs_->size();

    // Count valid trajectories from OTHER drones
    int valid_others = 0;
    for (int id = 0; id < std::min(traj_size, formation_size_); id++)
    {
      if (id != drone_id_ && swarm_trajs_->at(id).drone_id >= 0)
        valid_others++;
    }

    // Need all formation_size_-1 other drones to have valid trajectories
    if (valid_others < formation_size_ - 1)
      return false;
    ...
    for (int id = 0; id < formation_size_; id++)
    {
      if (id == drone_id_)
        continue;
      if (id >= traj_size || swarm_trajs_->at(id).drone_id < 0)
        continue;
  ...
  {
    if ((int)swarm_trajs_->size() < formation_size_ || !use_formation_)
    {
      return false;
    }
    else
  ...
  void PolyTrajOptimizer::showFormationInformation(bool is_show, Eigen::Vector3d pos)
  {
    ...
    if (drone_id_ != 0 || !use_formation_)
    {
      return;
    }
    // Check all drones have valid trajectories
    int valid = 1;
    for (int id = 0; id < (int)swarm_trajs_->size(); id++) {
      if (id != drone_id_ && swarm_trajs_->at(id).drone_id >= 0) valid++;
    }
    if (valid < formation_size_) return;
  ...
  void PolyTrajOptimizer::setParam(rclcpp::Node::SharedPtr node)
  {
    ...
    if (!node_->has_parameter("optimization/constrain_points_perPiece")) {
      node_->declare_parameter<int>("optimization/constrain_points_perPiece", -1);
    }
    node_->get_parameter("optimization/constrain_points_perPiece", cps_num_prePiece_);

    if (!node_->has_parameter("optimization/weight_obstacle")) {
      node_->declare_parameter<double>("optimization/weight_obstacle", -1.0);
    }
    node_->get_parameter("optimization/weight_obstacle", wei_obs_);

    if (!node_->has_parameter("optimization/weight_swarm")) {
      node_->declare_parameter<double>("optimization/weight_swarm", -1.0);
    }
    node_->get_parameter("optimization/weight_swarm", wei_swarm_);

    if (!node_->has_parameter("optimization/weight_feasibility")) {
      node_->declare_parameter<double>("optimization/weight_feasibility", -1.0);
    }
    node_->get_parameter("optimization/weight_feasibility", wei_feas_);

    if (!node_->has_parameter("optimization/weight_sqrvariance")) {
      node_->declare_parameter<double>("optimization/weight_sqrvariance", -1.0);
    }
    node_->get_parameter("optimization/weight_sqrvariance", wei_sqrvar_);

    if (!node_->has_parameter("optimization/weight_time")) {
      node_->declare_parameter<double>("optimization/weight_time", -1.0);
    }
    node_->get_parameter("optimization/weight_time", wei_time_);

    if (!node_->has_parameter("optimization/weight_formation")) {
      node_->declare_parameter<double>("optimization/weight_formation", -1.0);
    }
    node_->get_parameter("optimization/weight_formation", wei_formation_);

    if (!node_->has_parameter("optimization/obstacle_clearance")) {
      node_->declare_parameter<double>("optimization/obstacle_clearance", -1.0);
    }
    node_->get_parameter("optimization/obstacle_clearance", obs_clearance_);

    if (!node_->has_parameter("optimization/swarm_clearance")) {
      node_->declare_parameter<double>("optimization/swarm_clearance", -1.0);
    }
    node_->get_parameter("optimization/swarm_clearance", swarm_clearance_);

    if (!node_->has_parameter("optimization/formation_type")) {
      node_->declare_parameter<int>("optimization/formation_type", -1);
    }
    node_->get_parameter("optimization/formation_type", formation_type_);

    if (!node_->has_parameter("optimization/max_vel")) {
      node_->declare_parameter<double>("optimization/max_vel", -1.0);
    }
    node_->get_parameter("optimization/max_vel", max_vel_);

    if (!node_->has_parameter("optimization/max_acc")) {
      node_->declare_parameter<double>("optimization/max_acc", -1.0);
    }
